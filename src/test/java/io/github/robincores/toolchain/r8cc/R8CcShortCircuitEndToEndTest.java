...
public class R8CcShortCircuitEndToEndTest {
    ...
    @Test
    void logical_and_short_circuits_rhs_assignment() {
        String cSource = """
                int main() {
                    int x = 0;
                    x && (x = 1);
        ...
        assertEquals(0, cpu.a() & 0xFFFF, "rhs assignment must be skipped when lhs is zero");
    }

    @Test
    void logical_or_short_circuits_rhs_assignment() {
        String cSource = """
                int main() {
                    int x = 1;
                    x || (x = 0);
        ...
        assertTrue(cpu.isHalted(), "program should halt");
        assertEquals(1, cpu.a() & 0xFFFF, "rhs assignment must be skipped when lhs is non-zero");
    }
}
