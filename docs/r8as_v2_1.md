## R8 Assembler (r8as) User Manual — v2.1
...
### Changelog (v2 → v2.1)
- Added **`.ascii "text"`** as an alias of **`.string "text"`** (same escapes, no automatic NUL terminator).
...
.ascii  "a;b" ; same behavior as .string
```
...
- Emits **one word per character**.
- **No automatic NUL terminator**.
- Same escape sequences as `.string`.
- In `.bss`, it **reserves** space (advances `ip`) without emitting.

Example:
```asm
.data
banner: .ascii "R8 READY\n"
        .byte 0
```
